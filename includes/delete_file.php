...
function deleteShare($key1,$con){
    ...
    $shanchu2="delete from sd_file where key1 = '$key1'";

    mysqli_query($con,$shanchu);
	mysqli_query($con,$shanchu1);
    mysqli_query($con,$shanchu2);
}
switch ($policyType) {
	case 'qiniu':
		Qiniu_SetKeys($ak, $sk);
    ...
    @unlink(dirname(dirname(__FILE__))."/".$plistDir."/".$plistname);
    @unlink(dirname(dirname(__FILE__))."/".$plistDir."/".$iconname);
?>