...
$numq=mysqli_query($con,"SELECT * FROM `sd_category`");
...
$line="SELECT * FROM `sd_category` ORDER BY `id` DESC limit $page $pagesize";
...
if($pageval=="1"){
    ...
    $pre =  $url."?id=categorylist&page=".($pageval-1);
}
if ($pageval==$pagenum){
    $ne =  "##";
}else{
    $ne =  $url."?id=categorylist
    
    
    &page=".($pageval+1);
}

if (isset($_GET['model']) == "getapplist"){
...
$smarty->display("appcategory.html");
?>